...
for (let i = 0; i < 8; i++) {
  timebtn1[i + 8].classList.add("hidden");
  timebtn2[i + 8].classList.add("hidden");
  timebtn3[i + 8].classList.add("hidden");
  timebtn4[i + 8].classList.add("hidden");
  timebtn5[i + 8].classList.add("hidden");
  timebtn6[i + 8].classList.add("hidden");
}
var targetSVG =
...
flightselector.addEventListener("click", (e) => {
  selectedFlight = flightselector.options[flightselector.selectedIndex].value;
});
...
for (let i = 0; i < productions.length; i++) {
  ...
  productions[i].addEventListener("click", (e) => {
    chosenPlay = productions[i].dataset.value;
    ...
    for (let i = 0; i < 8; i++) {
      timebtn1[i + 8].classList.remove("hidden");
      timebtn2[i + 8].classList.remove("hidden");
      timebtn3[i + 8].classList.remove("hidden");
      timebtn4[i + 8].classList.remove("hidden");
      timebtn5[i + 8].classList.remove("hidden");
      timebtn6[i + 8].classList.remove("hidden");
    }
    timeButtons();
...
function occupySelected(
  ...
  xml.onreadystatechange = function () {
    if (this.readyState == 4 && this.status == 200) {
      ...
      a = xmlDoc.getElementsByTagName("seats")[0];
      ...
      newElement1.appendChild(xmlDoc.createTextNode(username));
      newElement2.appendChild(xmlDoc.createTextNode(password));
      newElement3.appendChild(xmlDoc.createTextNode(covidStatus));
      newElement4.appendChild(xmlDoc.createTextNode(vaxStatus));
      newElement5.appendChild(xmlDoc.createTextNode(cardHolder));
      newElement6.appendChild(xmlDoc.createTextNode(cardNo));
  ...
  xml.open("GET", "database.xml", true);
  xml.send();
}
...
btnSubmit.addEventListener("click", (e) => {
  ...
  var covidStat = covidCheck(covidTemp);
  if (chosenPlay == "" && chosenMovie == "" && selectedFlight == "") {
...
function addElement(
  ...
  var text =
    ...
    `<covidStatus>${covidStatus}</covidStatus>` +
    `<vaxStatus>${vaxStatus}</vaxStatus>` +
  ...
  xhttp.onreadystatechange = function () {
    if (this.readyState == 4 && this.status == 200) {
      ...
      var x = xmlDoc.getElementsByTagName("seats")[0].childNodes[0];
      x.insertData(0, text);
      console.log(x.data);
    }
  };
